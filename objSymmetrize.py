...
def symmetrize(x,y,sideReparamPoints=200,rep=0,KarcherMeanScale=True,cutEnds=True,reparamPoints=250,scaledHeightWidth=3,scaleDirection="both"):
    ...
    if cutEnds == True:
        new_x,new_y = cut_ends(new_x,new_y)
    new_xs,new_ys = rescale(np.array(new_x),np.array(new_y),scaledHeightWidth=scaledHeightWidth,scaleDirection=scaleDirection)
